...
function construccionCasas(bolsas) {
  ...
  return function construcciones(c){
    if(bolsas < 10) return "No se puede construir casas con esa cantidad de bolsas"
    if(c <= 0) return "Por favor ingresar cuantas casas quieres construir"
    if(bolsas / 10 < c) return `Solo puedes construir esta cantidad de casas: ` + bolsas / 10
    if(bolsas / 10 === c) return true
  }
